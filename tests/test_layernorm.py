...
class TestLayerNorm:
    ...
    def test_dx_backward_match(self, batch_size, seq_len, hidden_size):
        # Setup
        torch.manual_seed(42)
        ...
        # Assert input gradients (dx) match
        torch.testing.assert_close(
            x.grad,  # gradient of x from Triton LayerNorm
            x.grad,  # gradient of x from PyTorch LayerNorm
            rtol=1e-0,
            atol=1e-0,
            msg="LayerNorm input gradients (dx) don't match!"
        )
