...
def test_numerical_stability():
    ...
    seed = torch.tensor(42, device='cuda')
    output = TritonDropout.apply(x, 0.5, seed)
    
    # Check that we don't have any NaN or inf values
    assert not torch.isnan(output).any()
    assert not torch.isinf(output).any()
    
    # Non-zero values should be scaled correctly
    nonzero_vals = output[output != 0]
    if len(nonzero_vals) > 0:
        assert torch.allclose(torch.abs(nonzero_vals), torch.abs(x[output != 0]) * 2.0, rtol=1e-5)
...
def test_memory_efficiency():
    ...
    pytorch_mem = measure_memory(lambda: torch.nn.functional.dropout(x, p=0.5, training=True))
    
    # Measure our implementation memory usage
    triton_mem = measure_memory(lambda: TritonDropout.apply(x, 0.5, seed))
